...
} from "@/features/world-memory/location/places/legacyMap";

/**
 * Public pin  Enever expose Firebase UID or Firestore document id.
...
> {
  const { findCountry } = await import("@/features/world-memory/location/locations");
...
> {
  const { findCountry, findRegion } = await import("@/features/world-memory/location/locations");
...
/** Counts only for a country  ELocation shape comes from static JSON on the client. */
...
/**
 * TTL cleanup  Esame path as admin delete:
 * expired Trace ↁEdelete doc ↁEdecrement location ↁErebuild aggregates/stars.
 */
...
/**
 * Admin / single-Trace delete  Eidentical aggregate sync as TTL cleanup.
 */
...
/** Literary story footprints  Efixed MIAV IDs, city-level only. Idempotent. */
 ...
 * Character names are never stored  Emessage + place only.
 */
